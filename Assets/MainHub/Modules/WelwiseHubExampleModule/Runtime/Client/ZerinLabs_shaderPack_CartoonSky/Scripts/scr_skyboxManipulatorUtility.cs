...
namespace WelwiseHubExampleModule.Runtime.Client.ZerinLabs_shaderPack_CartoonSky.Scripts
{
    ...
    */
        ...
        private void Start()
        {
            ...
            {
                Debug.Log(
                    "WARNING: no animator assigned to this object. You need an animator to animate the skybox parameters!");
            }
        ...
        private void Update()
        {
            if (hasAnimator == true)
            {
                if (RenderSettings.skybox.shader.name.Contains(shaderName) == true)
                {
                    ...
                    {
                        DynamicGI
                            .UpdateEnvironment(); //WARNING!! this function is expensive, was set here for demo purposes only!
                    }
                }
            }
        }
